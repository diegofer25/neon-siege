...
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>Verify your Neon Siege account</title>
</head>
<body style="margin:0;padding:0;background:#0a0a0f;font-family:'Segoe UI',Arial,sans-serif;">
  <table width="100%" cellpadding="0" cellspacing="0" style="background:#0a0a0f;padding:40px 0;">
    <tr>
      <td align="center">
        <table width="480" cellpadding="0" cellspacing="0" style="background:#0f0f1a;border:1px solid rgba(0,255,255,0.15);border-radius:12px;padding:40px 36px;">
          ...
          <tr>
            <td style="color:#ccc;font-size:15px;line-height:1.6;padding-bottom:20px;">
          ...
          <tr>
            <td align="center" style="padding:8px 0 24px;">
              <div style="display:inline-block;padding:14px 22px;border:2px solid #00ffff;border-radius:8px;color:#00ffff;font-size:30px;font-weight:700;letter-spacing:10px;line-height:1;">
                ${code}
              </div>
            </td>
          </tr>
...
export async function sendRegistrationCodeEmail(to: string, code: string): Promise<void> {
  if (!resend) {
    console.log(`\n[email.service] DEV MODE — registration code for ${to}:\n  ${code}\n`);
    return;
  }
  ...
  if (error) {
    ...
    if (env.NODE_ENV !== 'production' && statusCode === 403 && name === 'validation_error') {
      console.warn('[email.service] Resend sandbox restriction. Falling back to logged registration code in development.');
      console.warn('[email.service] To send to any recipient, verify a domain in Resend and set EMAIL_FROM to that domain.');
      console.log(`\n[email.service] DEV FALLBACK — registration code for ${to}:\n  ${code}\n`);
      return;
    }
